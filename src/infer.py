...
def main(args):
    ...
    if os.name == 'nt':
        ...
        test_data = SmartathonImageDataset(args.data_dir+'\\test.csv', img_dir, transform=transforms, training=False)
    else:
        img_dir = args.data_dir+'/resized_images/'
        test_data = SmartathonImageDataset(args.data_dir+'/test.csv', img_dir, transform=transforms, training=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', '--model_type', dest='model_type', 
        help='type of model to test, see model.py for supported model types')
